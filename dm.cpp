    ...
    struct Job {
      int i, j, period, releaseTime, extime, deadline;
    };
    ...
    int main(){
        ...
        for (int i=1;i<=noOfTasks;i++){
           ...
           for (int j=1;tasks[i].phase + (j-1)*tasks[i].period<MAXTIME; j++){ // j denotes the job no of corr. task
                  tempJob.i = i;
                  tempJob.j = j;

                  tempJob.releaseTime = tasks[i].phase + (j-1)*tasks[i].period;
                  tempJob.extime = tasks[i].extime;
                  tempJob.deadline = tasks[i].deadline; // for DM we just need relative deadline
                  tempJob.period = tasks[i].period;
                  //jobs.push(tempJob);
        ...
        for (int t=0;t<=MAXTIME;t++){
                ...
                if (currentJob.extime<=0){
                     ...
                     // check if there is any job in the job queue
                       if (!jobs.empty()){
                        currentJob = jobs.top();
                        jobs.pop();
                        cout<<"job ("<<currentJob.i<<","<<currentJob.j<<") started execution at time:"<<t<<endl;
               ...
               // at this second point one unit of the job will be executed
               if(currentJob.extime + t>currentJob.deadline){
                ...
                break;
                   ...
                   currentJob.extime = currentJob.extime - 1;
